...
unsigned long timerDelay = 10000; // Timer set to 10 seconds
String jsonBuffer;

const long utcOffsetInSeconds = 19800; // UTC offset for Indian Standard Time (IST)
...
void setup(void) {
  ...
  tft.drawBitmap(9, 73, humid, 22, 30, 0x07FF);
...
void loop() {
  if ((millis() - lastTime) > timerDelay) {
    if (WiFi.status() == WL_CONNECTED) {
      ...
      temp = (temp - 273);  // Corrected Kelvin to Celsius conversion
      ...
      tft.setCursor(72, 42);
      if (temp < 10) {
        tft.print("0"); // Print leading zero if temperature is less than 10
      }
      tft.print(temp);
      ...
      if (hours < 10) {
        tft.print("0"); // Print leading zero if hours is less than 10
      }
      tft.print(hours); // Display hours in 12-hour format
      tft.print(":");
